...
const adapter = new PrismaMariaDb(
  process.env.DATABASE_URL ?? {
    ...
    database: process.env.DB_NAME ?? "coquette",
  }
);
...
const kelasReferensiSeed: {
  ...
  { label: "10 PPLG PLUS", jenjang: "SMK", tingkat: 10, jurusan: "PPLG" },
...
async function main() {
  ...
  await db.user.upsert({
    where: { email: "admin@coquette.sch.id" },
    update: {},
    create: {
      email: "admin@coquette.sch.id",
  ...
  await db.user.upsert({
    where: { email: "kepsek@coquette.sch.id" },
    update: {},
    create: {
      email: "kepsek@coquette.sch.id",
  ...
  await db.user.upsert({
    where: { email: "kurikulum@coquette.sch.id" },
    update: {},
    create: {
      email: "kurikulum@coquette.sch.id",
...
// Pastikan fungsi main dipanggil di paling bawah file kamu
main()
  .catch(async (e) => {
    console.error(e);
    process.exit(1);
