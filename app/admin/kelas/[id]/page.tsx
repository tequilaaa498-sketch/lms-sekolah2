...
export default function AdminKelasDetailPage() {
  ...
  async function handleHapusGuru(guruId: string, mapelId: string, kelasIdsLama: string[]) {
    if (!confirm("Keluarkan guru ini dari kelas?")) return;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      >
        <div className="min-h-full bg-white p-4">
          <p className="mb-3 px-2 pt-2 text-sm font-bold text-[#111827]">
            Dashboard Admin
            <br />
            <span style={{ color: BRAND }}>- Detail Kelas</span>
          </p>
            ...
            {ADMIN_TABS.map((tab) => (
              <button
                ...
                className={`flex cursor-pointer items-center gap-3 rounded-lg px-4 py-2.5 text-left text-sm font-semibold ${
                  tab.key === "KELAS" ? "text-gray-900" : "text-[#374151] hover:bg-black/5"
                }`}
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-6 sm:px-6">
        <div className="overflow-hidden border border-gray-300 bg-white text-gray-900">
          <div className="flex flex-wrap items-start justify-between gap-3 p-5">
            <div>
              <p className="text-lg font-bold">{kelas.judul}</p>
              {kelas.deskripsi && <p className="mt-1 text-sm text-gray-600">&quot;{kelas.deskripsi}&quot;</p>}
            </div>
            <div className="border border-gray-300 px-3 py-2 text-right">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500">Kode Kelas</p>
              <p className="text-sm font-bold">{kelas.inviteToken}</p>
              <button
                onClick={handleCopyInvite}
                className="mt-1 flex items-center gap-1 text-[11px] font-medium text-gray-700 hover:underline"
              >
          ...
          <div className="border-t border-gray-300 px-5 py-2.5">
            <button onClick={() => setShowEditKelas(true)} className="cursor-pointer text-xs font-semibold text-gray-700 hover:underline">
              Edit Kelas
            </button>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {kelas.feed.length === 0 ? (
                ...
                kelas.feed.map((item, i) => {
                  ...
                  const a = item.data;
                  return (
                    <div key={`a-${i}`} className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm">
                      <div className="flex items-center gap-2">
      ...
      <ModalKelas open={showEditKelas} onClose={() => setShowEditKelas(false)} onSuccess={loadKelas} mode="edit" initialData={kelas} />
